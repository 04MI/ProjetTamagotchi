...
WIDTH = 124#int(800/fx.5)
HEIGHT = 124#int(600/fy)
#sizes=[(10, 10), (10, 9), (5, 4), (8, 20), (5, 14), (4, 5), (2, 6), (4, 6), (13, 8), (27, 18), (27, 18), (13, 19), (13, 19), (13, 19), (13, 19), (13, 19), (18, 12), (17, 12), (276, 20), (7, 10)] 
fx=6
fy=5
...
class Dino(PhotoImage):
    def __init__(self, container, images):
        ...
        super().__init__()
        self.x,self.y=200//fx, HEIGHT - 100//fy#these are the positions of dino when game start
        print(self.x,self.y)
    ...
    def jump_dino(self):####This function will let run the animation of dinosaurus jump
        ...
        if not self.lean:
            ...
            if self.dir=='up':#First dino will move up 
                x,y=0,-20//fy#its y coordinates will decrease
                y=y+self.acc*2//fy#This will give it some acceleration
                self.y+=y#New value of y will be store
                #print(y,self.y)
                self.acc+=0.5#acceleration
                if self.y<=HEIGHT-250//fy:#check if dino has reached to some height
                    #exit()
                ...
                x,y=0,8/fy#initial y and it will increase as it is accelreting
                y=y+self.acc*2//fy#accleration of y
                self.y+=y#new value of y
                #print(y,self.y)
                self.acc+=0.5#acceleration
                if self.y>=HEIGHT-100//fy:#if dino has reached it initial position
                    y=-(self.y-(HEIGHT-100//fy))
                    #print(y,'y')
                    # y=self.y
                    # x=self.container.coords(self.dino_frames)[0]
                    self.y=HEIGHT-100//fy#then y position will be set to ground
                    ...
                    self.container.delete(self.dino_frames)
                    self.dino_frames = self.container.create_image(self.x,self.y, image=self.images[0])#creating images of dino
                    # self.container.moveto(self.dino_frames, x, y)
                    return
...
class Nature(PhotoImage):
    def __init__(self, container, image):
        ...
        super().__init__()
        self.create_cactus=[self.container.create_image(WIDTH / 3+100//fx, 495//fy, image=self.image[4]),
            self.container.create_image(WIDTH / 3, 495//fy, image=self.image[5]),
            self.container.create_image(WIDTH / 2+280//fx, 495//fy, image=self.image[6]),
            self.container.create_image(WIDTH / 2+380//fx, 495//fy, image=self.image[7])]#random.randrange(400, 450)
        self.create_ground = self.container.create_image(WIDTH/2, HEIGHT - 50//fy, image=self.image[0])
        self.create_clouds = [self.container.create_image(WIDTH / 2, random.randrange(150//fy, 400//fy), image=self.image[1]),
                              self.container.create_image(WIDTH/4, random.randrange(150//fy, 400//fy), image=self.image[2]),
                              self.container.create_image(WIDTH-150//fx, random.randrange(150//fy, 400//fy), image=self.image[3])]
    ...
    def animate_ground(self):
        ...
        self.container.move(self.create_ground, 800//fx, 0) if x <= 0 else self.container.move(self.create_ground, -5, 0)#moving ground to new location
        ...
        for cloud in self.create_clouds:#self.clouds:#animating clouds
            ...
            if x <= 0:#if it is leser than 0
                self.container.move(cloud, 850//fx, 0)#if cloud has reahced left most shift it to right most so that it can come in screen again
            else:#otherwise
        ...
        for cactus in self.create_cactus:#self.clouds:#animating clouds
            ...
            if x <= 0:#if it is leser than 0
                self.container.move(cactus, 850//fx, 0)#if cloud has reahced left most shift it to right most so that it can come in screen again
            else:#otherwise
...
class Obstacle(PhotoImage):
    def __init__(self, container, image):
        ...
        self.combDict={'1':(54//fx,101//fy),'2':(37//fx,70//fy),'3':(49//fx,50//fy),'11':(54*2//fx,101//fy,54//fx),'12':(54//fx+37//fx,101//fy,(54//fx+37//fx)/2),'21':(54//fx+37//fx,101//fy,(54//fx+37//fx)/2),'22':(37*2//fx,70//fy,37//fx),'4':(68//fx,50//fy)}#This dictionary contains data size of different images
        #self.animate_obstacles()
    def create(self,obs,offset):
        ind=0
        for typ in obs:
            if not self.ind:
                self.obsMov=WIDTH-100//fx#to place new obstacles
                self.ind=True
            if typ=='1':
                #creating image of big cactus
                self.ObsSets.append( ['1',self.container.create_image(WIDTH-100//fx+ind*offset//fx, HEIGHT-100//fy, image=self.image[1])])
            if typ=='2':
                #creating image of small cactus
                self.ObsSets.append(['2',self.container.create_image(WIDTH-100//fx+ind*offset//fx, HEIGHT-90//fy, image=self.image[0])])
            if typ=='3':
                #creating rocks
                self.ObsSets.append(['3',self.container.create_image(WIDTH-100//fx+ind*offset//fx, HEIGHT-70//fx, image=self.image[2])])
            if typ=='4':
                ...
                self.ObsSets.append(['4',self.container.create_image(WIDTH-50//fx+ind*offset//fx, random.choice([HEIGHT-150//fy,HEIGHT-100//fy]), image=self.image[self.birdView])])#Generating two random birds at differnt positions
            ind+=1
    ...
    def animate_obstacles(self,obs):#Animating differnt obstacles
        ...
        self.container.delete(self.ObsSets.pop(0)[1]) if x <= 0 else self.container.move(obs[1], -15//fy, 0)#deleting obstacle if it reaches the end
        try:#just to handle error
            if self.ObsSets.index(obs)==len(self.ObsSets)-1 and x<300//fx:#if obstacle is last one
        ...
        if len(self.ObsSets)<1 or (x<400//fx and not len(self.ObsSets)>2 and not self.bird ):#checking if obstacles sets lits is less than 1 or x for obstacle is less than 160 and obstacles are not 2 and also not bird
...
class DinoGame(Tk):
    ...
    def Start(self):
        ...
        self.spd=50//fx#speed of obstacles and ground
        self.spdD=50//fx#speed of dino
        ...
        self.Scor=self.canvas.create_text(740//fx-20, 580//fy+2, text=f'{self.score}'.zfill(4), fill="black", font=(f'Helvetica {15//fy+3} bold'))#Displaying score of the game
        self.canvas.create_text(100//fx+15, 20//fy, text='High Score:'+f'{self.HighScore}'.zfill(4), fill="white", font=(f'Helvetica {15//fy+3} bold'))#Displaying High score on the screeen
        #self.canvas.create_text(350//fx, 580//fy, text=f'Dino Run|  Q: Quit      P: Pause    R: Restart      Esc: Back to Tamagotchi', fill="black", font=(f'Helvetica {10//fy} bold'))#Displaying some short keys of the game
        self.b_next =  Button(self, text="next", command=self.end)
        self.b_next.grid(row=1, column=0)

        self.b_select = Button(self, text="select", command=lambda :self.dino.lean_dino() if self.animating else None)
        self.b_select.grid(row=1, column=1)

        self.b_previous = Button(self, text="back", command=lambda :self.dino.jump_dino() if self.animating else None)
    ...
    def end(self):
        ...
        #self.game.game.gameStart=False
        self.game.game.__init__(self.game.game.tickrate,self.game.game.tamagotchi)
        self.game.game.run()
        #self.game.game.mainWindow()
    ...
    def Stop(self,key):
        ...
        if not self.animating:#if animation variable is false
            self.pause=self.canvas.create_text(self.width/2, self.height/2, text=txt, fill="black", font=(f'Helvetica {35//fy} bold'))#display the message on the screen
        else:#othe wise 
    ...
    def animations(self):
        ...
        if self.animating:#checking if the game is allowed to animate looking at the animating flag
            ...
            if self.counter%10000==0 and self.counter!=0:#if counter has passed 10000 then update the time zone and background color
                ...
                if self.spd<10//fy:#if spd reaches a certain value don't further update it
                    self.spd=10//fy#keep it fix
        ...
        if not self.collide and not self.restart:#if it is neither collison and nor restart 
            self.ani=self.after(10, self.animations)#continue animation
    ...
    def overLp(self,othr,obs,w,h):#This method will check wheter the two rectangles overlap or not
        ...
        margins=15//fy#a margin value to help user giving some margin in collisoin
        if obs[0]=='4':#if bird is obstacle
            margins=8//fy#change margin a little
        obs=obs[1]
        try:#to prevent error
            x,y=othr.container.coords(obs)#getting coordinates of the obstacles
            dinoH=95//fx#dino height for upright position
            dinoW=88//fy#width of the upright dino
            if self.dino.lean:#if it is ducked
                dinoH=60//fx#heigth will be changed
                dinoW=118//fy#width of the bend dino
            #print(dinoH,margins)
